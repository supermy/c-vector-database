...
pub struct HnswIndex {
    ...
    id_to_idx: RwLock<ahash::AHashMap<u64, NodeId>>,
...
impl HnswIndex {
    ...
    pub fn with_config(dimension: usize, metric: DistanceMetric, config: HnswConfig) -> Self {
        Self {
            ...
            id_to_idx: RwLock::new(ahash::AHashMap::new()),
    ...
    pub fn insert(&self, external_id: u64, vector: Vec<f32>) {
        ...
        let node = Node::new(node_id, external_id, vector.clone(), level, self.config.m);
        nodes.push(node);
        drop(nodes);
        ...
        for lc in (0..=level.min(max_level)).rev() {
            let neighbors = self.search_layer(&nodes, &vector, curr_ep, self.config.ef_construction, lc);
            let selected = self.select_neighbors(&nodes, &vector, &neighbors, self.config.m);
            ...
            for &neighbor_id in &selected {
                ...
                if neighbor_conn.len() > self.config.m_max {
                    ...
                    *neighbor_conn = candidates.iter().take(self.config.m).map(|c| c.id).collect();
                }
            }
